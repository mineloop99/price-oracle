...
contract PriceOracle is Ownable {
    ...
    function update(address _token) public {
        ...
        if(timeElapsed >= PERIOD)   {
            ...
            _tokenInfoFallBack.price0Average = FixedPoint.uq112x112(uint224((price0Cumulative - _tokenInfoFallBack.price0CumulativeLast) / timeElapsed));
            _tokenInfoFallBack.price1Average = FixedPoint.uq112x112(uint224((price1Cumulative - _tokenInfoFallBack.price1CumulativeLast) / timeElapsed));
    ...
    function _estimateAmountTwap(
        ...
        if (tokenIn == pair.token0()) {
            amountOut = int256(int112(tokenInfoFallBacks[tokenIn].price0Average.decode()));
        } else {
            require(tokenIn == pair.token1(), "INVALID_TOKEN");
            amountOut = int256(int112(tokenInfoFallBacks[tokenIn].price1Average.decode()));
        }
